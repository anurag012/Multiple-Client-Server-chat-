...
import java.util.Iterator; //create the GUI of the client side 
public class MyClient extends WindowAdapter implements ActionListener{
 ...
 //create outputstream
 dout=new DataOutputStream(s.getOutputStream()); //sending a message for login 
 ...
 try{
	 ...
	 if (e.getSource()==lout){
		 frame.dispose(); 
		 //new Welcome(); 
		 JOptionPane.showMessageDialog(null,"Exit Chat Room"); 
		 try{
...
class My1 implements Runnable{ 
		...
		try{ 
			...
			while(true){
				...
				while(i1.hasNext()){ 
						...
						i++; //add the user names in list box
 ...
 class My implements Runnable{ 
	 ...
	 public void run(){
		 ...
		 while(true){ 
			 try{ 
				 str1=din.readUTF(); // receive the message // add the message in list box
